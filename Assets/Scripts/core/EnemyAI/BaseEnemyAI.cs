...
namespace EnemyAI
{
  ...
  public class BaseEnemyAI : MonoBehaviour
  {
    ...
    [Tooltip("If true, patrol points will be chosen randomly instead of sequentially")]
    [SerializeField] protected bool randomPatrolOrder = false;

    [Header("Pathfinding Obstacle Settings")]
    [Tooltip("Layers that block pathfinding. Used to check if patrol points are reachable.")]
    [SerializeField] protected LayerMask pathfindingObstacleLayers = ~0; // Default: all layers

    [Tooltip("Maximum attempts to find a reachable patrol point before skipping")]
    [SerializeField] protected int maxPatrolPointAttempts = 5;

    [Tooltip("If true, will validate patrol points on startup and remove unreachable ones")]
    [SerializeField] protected bool validatePatrolPointsOnStart = true;
    ...
    // Random patrol tracking
    protected List<int> availablePatrolIndices = new();
    protected List<int> visitedPatrolIndices = new();
    ...
    {
      patrolPoints.Clear();
      availablePatrolIndices.Clear();
      visitedPatrolIndices.Clear();
      ...
      else
      {
        ...
        // Validate patrol points if enabled
        if (validatePatrolPointsOnStart)
        {
          ValidatePatrolPoints();
        }

        // Initialize available indices for random patrol
        ResetAvailablePatrolIndices();
      }
    }

    /// <summary>
    /// Validates patrol points and removes unreachable ones
    /// </summary>
    protected virtual void ValidatePatrolPoints()
    {
      List<Transform> validPoints = new();

      foreach (Transform point in patrolPoints)
      {
        if (point != null && IsPatrolPointReachable(point.position))
        {
          validPoints.Add(point);
        }
        else
        {
          Log($"Patrol point '{point?.name}' is unreachable and will be skipped");
        }
      }

      if (validPoints.Count < patrolPoints.Count)
      {
        Log($"Removed {patrolPoints.Count - validPoints.Count} unreachable patrol points. {validPoints.Count} remaining.");
        patrolPoints = validPoints;
      }
    }

    /// <summary>
    /// Checks if a patrol point is reachable using A* pathfinding
    /// </summary>
    protected virtual bool IsPatrolPointReachable(Vector3 targetPosition)
    {
      // Use A* to check if path exists
      var path = ABPath.Construct(transform.position, targetPosition);
      AstarPath.StartPath(path);
      path.BlockUntilCalculated();

      // Path is reachable if it's not an error and has a valid path
      bool isReachable = !path.error && path.vectorPath != null && path.vectorPath.Count > 0;

      // Additional check: verify the path endpoint is close to target
      if (isReachable && path.vectorPath.Count > 0)
      {
        float distanceToTarget = Vector3.Distance(path.vectorPath[path.vectorPath.Count - 1], targetPosition);
        isReachable = distanceToTarget < patrolPointReachedDistance * 2f;
      }
    ...
    /// <summary>
    /// Checks if there's a clear line of sight (no obstacles) between two points
    /// </summary>
    protected virtual bool HasClearPath(Vector3 from, Vector3 to)
    {
      Vector3 direction = to - from;
      float distance = direction.magnitude;

      // Raycast to check for obstacles
      if (Physics.Raycast(from + Vector3.up * 0.5f, direction.normalized, out RaycastHit hit, distance, pathfindingObstacleLayers))
      {
        // Hit something in obstacle layer - path is blocked
        return false;
      }
    ...
    /// <summary>
    /// Resets the available patrol indices for random selection
    /// </summary>
    protected virtual void ResetAvailablePatrolIndices()
    {
      availablePatrolIndices.Clear();
      visitedPatrolIndices.Clear();
    ...
    protected virtual void SetNextPatrolPoint()
    {
      if (patrolPoints == null || patrolPoints.Count == 0)
      {
        Log("No patrol points available");
        return;
      }
      ...
      // Try to find a reachable patrol point
      while (attempts < maxPatrolPointAttempts && targetPoint == null)
      {
        int candidateIndex;
        ...
        {
          // Random patrol order
          candidateIndex = GetRandomPatrolIndex();
        }
        else
        {
          // Sequential patrol order
          candidateIndex = currentPatrolIndex;
        }

        if (candidateIndex < 0 || candidateIndex >= patrolPoints.Count)
        {
          Log("No more patrol points available");
          break;
        }

        Transform candidatePoint = patrolPoints[candidateIndex];

        if (candidatePoint != null)
        {
          // Check if patrol point is reachable
          if (IsPatrolPointReachable(candidatePoint.position))
          {
            targetPoint = candidatePoint;
            currentPatrolIndex = candidateIndex;

            // Mark as visited for random patrol
            if (randomPatrolOrder)
            {
              MarkPatrolIndexAsVisited(candidateIndex);
            }
          }
          else
          {
            Log($"Patrol point {candidateIndex} ('{candidatePoint.name}') is currently unreachable, trying next...");

            // For random patrol, remove this index from available
            if (randomPatrolOrder)
            {
              availablePatrolIndices.Remove(candidateIndex);
            }
            else
            {
              // For sequential, just move to next index
              currentPatrolIndex++;
              if (currentPatrolIndex >= patrolPoints.Count && loopPatrol)
              {
                currentPatrolIndex = 0;
              }
            }
      ...
      if (targetPoint != null)
      {
        aiPath.destination = targetPoint.position;
        Log($"Moving to patrol point {currentPatrolIndex} ('{targetPoint.name}')");
      ...
      {
        Log("Could not find any reachable patrol point after multiple attempts. Switching to idle.");
        fsm.ChangeState(EnemyState.Idle);
      }
    }

    /// <summary>
    /// Gets a random patrol index from available indices
    /// </summary>
    protected virtual int GetRandomPatrolIndex()
    {
      // If no available indices, reset the list
      if (availablePatrolIndices.Count == 0)
      {
        if (loopPatrol)
        {
          Log("All patrol points visited, resetting available indices");
          ResetAvailablePatrolIndices();
        }
        else
        {
          return -1; // No more points available and not looping
        }
      }
      ...
      // Pick a random index from available list
      int randomListIndex = UnityEngine.Random.Range(0, availablePatrolIndices.Count);
      return availablePatrolIndices[randomListIndex];
    }

    /// <summary>
    /// Marks a patrol index as visited (removes from available list)
    /// </summary>
    protected virtual void MarkPatrolIndexAsVisited(int index)
    {
      availablePatrolIndices.Remove(index);
      if (!visitedPatrolIndices.Contains(index))
      {
        visitedPatrolIndices.Add(index);
      }
    }
    ...
    protected virtual void MoveToNextPatrolPoint()
    {
      ...
      if (randomPatrolOrder)
      {
        // For random patrol, just call SetNextPatrolPoint which will pick a random one
        SetNextPatrolPoint();
        PlayPatrolSound();
      }
      else
